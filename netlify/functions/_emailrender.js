...
function renderBlocks(blocks, site) {
  ...
  return (blocks || []).map(bk => {
    switch (bk.type) {
      ...
      case 'give': return `<div style="background:#fbf7ef;border:1px solid #eadfc8;border-radius:12px;padding:16px 18px;margin:6px 0 14px"><b style="font-size:15px">Partner with us</b><p style="font-size:12.5px;color:#7a756f;margin:4px 0 12px">Every gift is designated for our work.</p><a href="${esc(bk.url || GIVE)}" style="display:inline-block;background:${/^#[0-9a-fA-F]{6}$/.test(bk.color || '') ? bk.color : '#FF6600'};color:#fff;font-weight:700;text-decoration:none;padding:11px 24px;border-radius:22px">${esc(bk.label || 'Give')}</a></div>`;
      case 'button': return bk.url ? `<p style="margin:6px 0 14px;text-align:${['center', 'right'].includes(bk.align) ? bk.align : 'left'}"><a href="${esc(bk.url)}" style="display:inline-block;background:${/^#[0-9a-fA-F]{6}$/.test(bk.color || '') ? bk.color : '#FF6600'};color:#fff;font-weight:700;text-decoration:none;padding:11px 24px;border-radius:9px">${esc(bk.label || 'Open')}</a></p>` : '';
