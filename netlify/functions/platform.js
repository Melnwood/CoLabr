...
exports.handler = async function (event) {
  ...
  if (event.httpMethod === 'GET') return r(200, { paused: !!rec.fields['Pause all email'] });
  ...
  const pause = !!b.pause;
  const ur = await fetch(api, { method: 'PATCH', headers: auth, body: JSON.stringify({ records: [{ id: rec.id, fields: { 'Pause all email': pause } }] }) });
  if (!ur.ok) return r(502, { error: 'Could not update settings.' });
  return r(200, { ok: true, paused: pause });
};
