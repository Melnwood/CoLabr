...
exports.handler = async function (event) {
  ...
  try {
    ...
    // Who is asking? The supporter with the right key, or the signed-in missionary who owns it.
    const keyOk = !!(key && c['Thread Key'] && key === c['Thread Key']);
    let isOwner = false;
    const sess = sessionFromEvent(event);
    if (!keyOk && sess && sess.email) {
      try { const me = await missByEmail({ Authorization: 'Bearer ' + token }, sess.email); isOwner = !!(me && me.name && me.name === (c['Missionary'] || '')); } catch (e) {}
    }
    if (!keyOk && !isOwner) return r(403, { error: 'This link is not valid.' });
      ...
      if (from === 's') {
        ...
        if (to) await sendMail({
          ...
          html: `<div style="font-family:-apple-system,Arial,sans-serif;max-width:520px;color:#241f1b">
            ...
            ${site ? `<p><a href="${site}/manage.html" style="color:#FF6600;font-weight:700">Reply in Co·labr →</a><br><span style="font-size:12px;color:#7a756f">Replying there keeps the whole conversation in one place.</span></p>` : ''}
          </div>`,
        ...
        await sendMail({
          ...
          html: `<div style="font-family:-apple-system,Arial,sans-serif;max-width:520px;color:#241f1b">
            ...
            ${(site && tkey) ? `<p style="margin:20px 0"><a href="${site}/thread.html?r=${id}&k=${tkey}" style="background:#FF6600;color:#fff;font-weight:700;text-decoration:none;border-radius:10px;padding:11px 20px;display:inline-block">Reply →</a></p>` : ''}
          </div>`,
