  ...
  for (const rc of recipients) {
    ...
    const cta = chooseUrl
      ? `<p style="margin:6px 0 10px"><a href="${chooseUrl}" style="display:inline-block;background:#FF6600;color:#fff;font-weight:700;font-size:15px;text-decoration:none;padding:12px 26px;border-radius:10px">Choose how you'd like to follow →</a></p>
         <p style="margin:0 0 18px;font-size:12.5px;color:#7a756f">One click — we already know it's you.${site ? ` Or just <a href="${site}/index.html?m=${encodeURIComponent(missionary)}&t=${token}" style="color:#FF6600">browse the updates</a> first.` : ''}</p>`
      : (site ? `<p style="margin:6px 0 18px"><a href="${site}/index.html?m=${encodeURIComponent(missionary)}&t=${token}" style="display:inline-block;background:#FF6600;color:#fff;font-weight:700;font-size:15px;text-decoration:none;padding:12px 26px;border-radius:10px">See our updates →</a></p>` : '');
